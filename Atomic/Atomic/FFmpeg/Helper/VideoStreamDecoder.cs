...
namespace FFmpeg.Helper
{
    public sealed unsafe class VideoStreamDecoder : IDisposable
    {
        ...
        public bool TryDecodeNextFrame(out AVFrame frame, long fromPos = 0L)
        {
            ffmpeg.av_frame_unref(_pFrame);
            int error;
        ...
        // Added
        //---------------------------------------------
        private bool _initFirstFrame = true;
        private void InitializeFirstFrame(long framePosition = 0)
        {
            if (_initFirstFrame == true)
            {
                ffmpeg.av_seek_frame(_pFormatContext, _streamIndex, framePosition, ffmpeg.AVSEEK_FLAG_BACKWARD);
                _initFirstFrame = false;
            }
        }

        public void FirstFrameReset()
        {
